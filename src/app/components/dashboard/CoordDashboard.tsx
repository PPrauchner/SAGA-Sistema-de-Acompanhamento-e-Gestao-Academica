...
function advisorRows(data: StudentsByAdvisorResponse) {
  return data.items.map((o) => ({
    ...
    orientandos: o.total_orientandos,                                                                                                                                                                                  regulares: o.regulares,
    risco: o.em_risco,                                                                                                                                                                                               }));
}                                                                                                                                                                                                                  
/** Agrega o histórico de conclusões por ano → média de meses (arredondada). */
function completionByYear(data: CompletionTimeResponse) {                                                                                                                                                            const byYear = new Map<number, { sum: number; count: number }>();
  for (const h of data.historico) {
    const cur = byYear.get(h.ano_conclusao) ?? { sum: 0, count: 0 };
    cur.sum += h.meses;
    cur.count += 1;                                                                                                                                                                                                    byYear.set(h.ano_conclusao, cur);
  }                                                                                                                                                                                                                  return [...byYear.entries()]
    .sort((a, b) => a[0] - b[0])                                                                                                                                                                                       .map(([ano, { sum, count }]) => ({ ano: String(ano), meses: Math.round(sum / count), con
}
...
function productionTotals(data: ProductionsReportResponse) {
  ...
  for (const s of data.por_aluno) {                                                                                                                                                                                    for (const nivel of PRODUCTION_LEVELS) totals[nivel] += s.por_nivel[nivel] ?? 0;
...
const ALERTS: AlertItem[] = [
  ...
  { id: "a3", nivel: "atencao", titulo: "Bolsas com expiração iminente", descricao: "6 bolsas CAPES/CNPq vencem nos próximos 30 dias e precisam de renovação.", afetados: 6, data: "02/06/2026", acao: "Processar renovações" },                                                                                                                                                                                                       { id: "a4", nivel: "atencao", titulo: "Orientadores com sobrecarga", descricao: "3 orientaecomendado de 8 orientandos simultâneos.", afetados: 3, data: "30/05/2026", acao:"Redistribuir orientações" },                                                                                                                                                                                        { id: "a5", nivel: "info", titulo: "Avaliação CAPES — Prazo para envio de dados", descricados da avaliação quadrienal é 30/07/2026.", afetados: 0, data: "28/05/2026" },
  { id: "a6", nivel: "info", titulo: "Novo edital de bolsas produtividade CNPq", descricao: "Edital aberto para bolsas PQ 2026. Prazo de inscrição: 15/07/2026.", afetados: 0, data: "29/05/2026" },               ];
                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                   const TIPO_CFG: Record<ValidationQueueItem["tipo"], { label: string; color: string; icon: Re
  producao: { label: "Produção", color: "#1F8A70", icon: <Award size={13} /> },
  atividade: { label: "Atividade", color: "#D4A017", icon: <BookOpen size={13} /> },                                                                                                                               };

// Chaveado pelos status do backend (ExtensionResponse.status); 'rejeitada' é exibido como "Negada".                                                                                                               const EXT_STATUS_CFG: Record<string, { label: string; color: string; bg: string }> = {
  pendente: { label: "Pendente", color: "#D4A017", bg: "#fffbeb" },
  em_analise: { label: "Em Análise", color: "#123C7A", bg: "#eef3fc" },                                                                                                                                              aprovada: { label: "Aprovada", color: "#1F8A70", bg: "#f0fdf4" },
  rejeitada: { label: "Negada", color: "#dc2626", bg: "#fef2f2" },                                                                                                                                                 };

// Formata data ISO (date "AAAA-MM-DD" ou datetime) em "DD/MM/AAAA", sem deslocar por fuso.                                                                                                                        function formatDataBR(value?: string | null): string {
...
function advisorRows(data: StudentsByAdvisorResponse) {
  return data.items.map((o) => ({
    ...
    regulares: o.regulares,                                                                                                                                                                                    risco: o.em_risco,
  }));                                                                                                                                                                                                     }

/** Agrega o histórico de conclusões por ano → média de meses (arredondada). */                                                                                                                            function completionByYear(data: CompletionTimeResponse) {
  const byYear = new Map<number, { sum: number; count: number }>();
  for (const h of data.historico) {
    const cur = byYear.get(h.ano_conclusao) ?? { sum: 0, count: 0 };
    cur.sum += h.meses;                                                                                                                                                                                        cur.count += 1;
...
/** Soma as produções aprovadas por nível de relevância entre todos os alunos. */
function productionTotals(data: ProductionsReportResponse) {                                                                                                                                                 const totals: Record<string, number> = { A1: 0, A2: 0, A3: 0, A4: 0, A5: 0, A6: 0, A7: 0,
  for (const s of data.por_aluno) {
    for (const nivel of PRODUCTION_LEVELS) totals[nivel] += s.por_nivel[nivel] ?? 0;                                                                                                                         }
  return PRODUCTION_LEVELS.map((nivel) => ({ nivel, total: totals[nivel] }));                                                                                                                              }
                                                                                                                                                                                                           const ALERTS: AlertItem[] = [
  { id: "a1", nivel: "critico", titulo: "Prazos vencidos sem prorrogação aprovada", descricao: "8 alunos ultrapassaram o prazo máximo de integralização sem prorrogação formalizada.", afetados: 8, data: "02/06/2026", acao: "Ver alunos" },                                                                                                                                                                          { id: "a2", nivel: "critico", titulo: "Relatórios semestrais em atraso", descricao: "12 reo foram entregues até a data limite.", afetados: 12, data: "01/06/2026", acao: "Notificaralunos" },
  ...
  { id: "a6", nivel: "info", titulo: "Novo edital de bolsas produtividade CNPq", descricao:  2026. Prazo de inscrição: 15/07/2026.", afetados: 0, data: "29/05/2026" },
];
...
function KpiCard({ icon, label, value, sub, color, trend }: {
  ...
  return (
    <div className="rounded-2xl p-5" style={{ background: "var(--card)", border: "1px solid var(--border)", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
      ...
      <p style={{ fontSize: "22px", fontWeight: 800, color: "var(--foreground)", lineHeight:x]">{value}</p>
      ...
      {(["pdf", "excel", "csv"] as ExportFormat[]).map((fmt) => (
        <button
          ...
          style={{
            ...
            fontSize: "10px", fontWeight: 700, border: `1px solid ${fmt === "pdf" ? "var(--t "excel" ? "var(--tint-teal-border)" : "var(--border)"}`,
          }}
        >
    ...
    <div className="flex flex-col gap-2 mb-5 sm:flex-row sm:items-center sm:justify-between"
      <div className="min-w-0 flex items-center gap-2">
        <div>
          ...
          {sub && <p style={{ fontSize: "11px", color: "var(--muted-foreground)", marginTop:
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {statusData.map((s) => {
              ...
              return (
              <div key={s.name} className="flex items-center justify-between p-2.5 rounded-xcolor}0d` }}>
                <div className="flex items-center gap-2">
                  <div className="rounded-full" style={{ width: 10, height: 10, background: s.color }} />
                  <span style={{ fontSize: "13px", fontWeight: 600, color: "var(--foreground
                </div>
                <div className="flex items-center gap-3">
                  <div className="rounded-full overflow-hidden" style={{ width: 80, height:  }}>
                    <div className="h-full rounded-full" style={{ width: `${(s.value / totalAlunos) * 100}%`, background: s.color }} />
                  </div>
                  <span style={{ fontSize: "13px", fontWeight: 700, color: s.color, minWidth
          ...
          <p style={{ fontSize: "12px", color: "var(--muted-foreground)", textAlign: "center", borderTop: "1px solid var(--border)", paddingTop: "12px" }}>
            Total: {statusData.reduce((acc, d) => acc + d.value, 0)} alunos matriculados · P
          </p>
        </div>
      ),
    },
        ...
        <div className="space-y-4">
          <ResponsiveContainer width="100%" height={Math.max(180, advisorList.length * 28)}>
            <BarChart data={advisorList} layout="vertical" margin={{ left: 10, right: 20 }}>
              ...
              <YAxis dataKey="name" type="category" tick={{ fontSize: 10, fill: "var(--muted>
              <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
              <Bar dataKey="orientandos" name="Orientandos" fill="#123C7A" radius={[0, 4, 4, 0]} barSize={6} isAnimationActive={false} />
              <Bar dataKey="regulares" name="Regulares" fill="#1F8A70" radius={[0, 4, 4, 0]}e={false} />
        ...
        <div className="space-y-4">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={productionByLevel}>
              ...
              <YAxis tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} allowDecimals=
              <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="rounded-xl p-3 text-center" style={{ background: "#123C7A0d", border: "1px solid #123C7A22" }}>
              <p style={{ fontSize: "20px", fontWeight: 800, color: "#123C7A" }}>{productionas}</p>
            ...
            {productionByLevel.map((s) => (
              <div key={s.nivel} className="rounded-xl p-3 text-center" style={{ background: `${LEVEL_COLORS[s.nivel]}0d`, border: `1px solid ${LEVEL_COLORS[s.nivel]}22` }}>
                ...
                <p style={{ fontSize: "10px", color: "var(--muted-foreground)", marginTop: "
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={completionYears}>
              ...
              <Tooltip formatter={(v: number) => [`${v} meses`, ""]} contentStyle={{ borderR
              <ReferenceLine y={META_INTEGRALIZACAO_MESES} stroke="#123C7A" strokeDasharray="4 4" label={{ value: "Meta", fill: "#123C7A", fontSize: 10 }} />
              <Bar dataKey="meses" name="Média (meses)" fill="#123C7A" radius={[4, 4, 0, 0]}
            </BarChart>
          </ResponsiveContainer>
          <div className="space-y-2">
              ...
              <div key={d.ano} className="flex items-center justify-between p-2.5 rounded-xl" style={{ background: "var(--muted)" }}>
                ...
                <div className="flex items-center gap-4">
                  <span style={{ fontSize: "12px" }}>
                    ...
                    <span style={{ fontWeight: 700, color: d.meses > META_INTEGRALIZACAO_MES>{d.meses}m</span>
                  </span>
                  <span style={{ fontSize: "12px" }}>
...
function StatusDistribChart({ onReport, statusData }: { onReport: () => void; statusData: St
        ...
        <div className="flex-1 space-y-2">
          {statusData.map((s) => (
            <div key={s.name} className="flex items-center gap-2">
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: s.color, f
              ...
              <div className="rounded-full overflow-hidden" style={{ width: 60, height: 5, background: "var(--muted)" }}>
                <div className="h-full rounded-full" style={{ width: `${(s.value / total) * } />
              </div>
...
function ProducaoChart({ onReport, data, loading, error }: {
  ...
  return (
    <div className="rounded-2xl p-4 md:p-5 overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <SectionHeader title="Produção por Nível" sub="Produções aprovadas por estrato Qualis" onReport={onReport} />
      {ready ? (
      <div className="overflow-x-auto -mx-1">
      ...
      <ResponsiveContainer width="100%" height={190}>
        <BarChart data={levels} margin={{ top: 4, right: 12, bottom: 0, left: 0 }}>
          ...
          <XAxis key="pr-x" dataKey="nivel" tick={{ fontSize: 10, fill: "var(--muted-foregro
...
function IntegralizacaoChart({ onReport, data, loading, error }: {
  ...
  return (
    <div className="rounded-2xl p-4 md:p-5 overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <SectionHeader title="Integralização por Ano" sub={`Média de meses vs. meta de ${META_ion="Integralização" onReport={onReport} />
      {ready ? (
      <div className="overflow-x-auto -mx-1">
      ...
      <ResponsiveContainer width="100%" height={190}>
        <BarChart data={years} margin={{ top: 4, right: 12, bottom: 0, left: 0 }}>
          ...
          <XAxis key="in-x" dataKey="ano" tick={{ fontSize: 11, fill: "var(--muted-foregroun
          <YAxis key="in-y" width={32} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} domain={[0, "auto"]} unit="m" />
          <Tooltip key="in-tip" formatter={(v: number) => [`${v} meses`, ""]} contentStyle={11 }} />
          <ReferenceLine key="in-rl1" y={META_INTEGRALIZACAO_MESES} stroke="#123C7A" strokeDasharray="4 4" strokeWidth={1.5} />
          <Bar key="in-b1" dataKey="meses" name="Média (meses)" fill="#123C7A" radius={[4, 4alse} />
        </BarChart>
      </ResponsiveContainer>
      </div>
      </div>
...
function ValidationQueue({ items, loading, error }: { items: ValidationQueueItem[]; loading: boolean; error: string | null }) {
  ...
  return (
    <div className="rounded-2xl p-4 md:p-5" style={{ background: "var(--card)", border: "1px
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 style={{ fontSize: "15px", fontWeight: 700, color: "var(--foreground)" }}>Fila
      ...
      {error ? (
        <p style={{ fontSize: "12px", color: "var(--tint-danger-text)" }}>Erro ao carregar a
      ) : loading ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-1.5 py-0.5 rounded" style={{ fontSize: "10px", fontW.color}18`, color: tc.color }}>{tc.label}</span>
                  </div>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <p style={{ fontSize: "11px", fontWeight: 700, color: "var(--foreground)"
                </div>
                ...
                <div className="px-4 pb-4 pt-0">
                  <div className="rounded-xl p-3 space-y-2" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button className="px-3 py-2 rounded-xl" style={{ background: "var(--moreground)", fontSize: "12px", fontWeight: 700, border: "1px solid var(--border)" }}>
                        <Eye size={13} />
                      </button>
                    </div>
                  </div>
...
function ExtensionRequestsSection({ extensions, loading, error }: { extensions: Solicitacao[]; loading: boolean; error: string | null }) {
  ...
  return (
    <div className="rounded-2xl p-4 md:p-5" style={{ background: "var(--card)", border: "1px
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 style={{ fontSize: "15px", fontWeight: 700, color: "var(--foreground)" }}>Soli
      ...
      {error ? (
        <p style={{ fontSize: "12px", color: "var(--tint-danger-text)" }}>Erro ao carregar p
      ) : loading ? (
        ...
        <div className="space-y-3">
          {extensions.map((ext) => {
            ...
            const sc = EXT_STATUS_CFG[status] ?? { label: status, color: "var(--muted-foregr
            ...
            return (
              <div key={ext.id} className="rounded-xl p-4" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center gap-2">
                      <span style={{ fontSize: "13px", fontWeight: 700, color: "var(--foregr
                ...
                {podeDecidir && (
                  <div className="flex gap-2 mt-3">
                    ...
                    <button className="px-4 py-1.5 rounded-lg" style={{ background: "var(--tint-danger-bg)", color: "var(--tint-danger-text)", fontSize: "11px", fontWeight: 700, border: "1px solid
var(--tint-danger-border)" }}>Negar</button>
                    <button className="px-4 py-1.5 rounded-lg" style={{ background: "var(--muted)", color: "var(--muted-foreground)", fontSize: "11px", fontWeight: 700, border: "1px solid var(--border)"
  ...
  return (
    <div className="rounded-2xl p-4 md:p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-center gap-2">
            <h3 style={{ fontSize: "15px", fontWeight: 700, color: "var(--foreground)" }}>Ce
          ...
          <p style={{ fontSize: "12px", color: "var(--muted-foreground)" }}>
            ...
            {" · "}{ALERTS.filter(a => a.nivel === "atencao").length} atenção · {ALERTS.filtength} informativos
          </p>
        </div>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span style={{ fontSize: "10px", color: "var(--muted-foreground)", flexSlert.data}</span>
                  </div>
                  <p style={{ fontSize: "11px", color: "var(--muted-foreground)", marginTop:lert.descricao}</p>
                  <div className="flex items-center gap-3 mt-2">
                    {alert.afetados > 0 && (
                      <span className="px-2 py-0.5 rounded-lg" style={{ fontSize: "10px", fo{ac.color}18`, color: ac.color }}>
      ...
      {ALERTS.length > 4 && (
        <button
          ...
          style={{ background: "var(--muted)", color: "var(--muted-foreground)", fontSize: "r: "1px solid var(--border)" }}
        >
...
export function CoordDashboard() {
  ...
  const { data: pendingExtensions, loading: extLoading, error: extError } = usePendingExtens
  ...
  const [completion, setCompletion] = useState<{ data: CompletionTimeResponse | null; error:l, error: null });
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      setCompletion(c.status === "fulfilled" ? { data: c.value, error: null } : { data: nullrregar a integralização." });
    ...
    return (
      <div className="flex items-center justify-center" style={{ minHeight: 400 }}>
        <div className="text-center">
          <div className="animate-spin rounded-full border-4 border-t-transparent" style={{ Color: "var(--border)", borderTopColor: "transparent" }} />
  ...
  if (dashData?.alunos_por_status) {
    statusData = [
      ...
      { name: "Prorrogação", value: dashData.alunos_por_status.em_prorrogacao, color: "#f973
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span style={{ fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.7pan>
            </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            {[
              ...
              { v: dashData?.prorrogacoes_pendentes ?? extensions.length, l: "Prorrogações",
            ].map((s) => (
      ...
      {/* KPI Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 md
        ...
        <KpiCard icon={<AlertTriangle size={20} />} label="Em Risco" value={emRisco} sub="Re#dc2626" trend="down" />
        ...
        <KpiCard icon={<TrendingUp size={20} />} label="Tempo Médio" value={dashData?.tempo_`${dashData.tempo_medio_integralizacao_meses.toFixed(1)}m` : "N/D"} sub="Integralização"color="#8b5cf6" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <ExtensionRequestsSection extensions={extensions} loading={extLoading} error={extErr
      </div>
      {/* Alerts */}
      <AlertsCenter />
